...
/* ============================================================
   MODULO DOMICILIACION BANCARIA v1.4
   Cada fila del Excel se divide en 15 cuotas en el TXT
   Solo visible para admin
   ============================================================ */

const DomiciliacionModule = (() => {
    let excelData = [];
    let txtContent = "";
    let fileName = "";
    const CUOTAS = 15;
    ...
    function init() {
        const card = document.getElementById("cardDomiciliacion");
        if (!card) return;
    ...
    function cacheRefs() {
        refs.dropzone = document.getElementById("domi-dropzone");
        refs.fileInput = document.getElementById("domi-file-input");
        refs.preview = document.getElementById("domi-preview");
        refs.resumen = document.getElementById("domi-resumen");
        refs.actions = document.getElementById("domi-actions");
        refs.progress = document.getElementById("domi-progress");
        refs.fileName = document.getElementById("domi-file-name");
        refs.fileSize = document.getElementById("domi-file-size");
        refs.statRegistros = document.getElementById("domi-stat-registros");
        refs.statLineas = document.getElementById("domi-stat-lineas");
        refs.statMonto = document.getElementById("domi-stat-monto");
        refs.btnPreview = document.getElementById("domi-btn-preview");
        refs.btnGenerar = document.getElementById("domi-btn-generar");
        refs.btnRemove = document.getElementById("domi-remove-file");
        refs.fechaNomina = document.getElementById("domi-fecha-nomina");
        refs.cuentaMatriz = document.getElementById("domi-cuenta-matriz");
        refs.codigoEmpresa = document.getElementById("domi-codigo-empresa");
        refs.operador = document.getElementById("domi-operador");
        refs.modal = document.getElementById("domi-modal-preview");
        refs.modalClose = document.getElementById("domi-modal-close");
        refs.modalBtnCerrar = document.getElementById("domi-modal-btn-cerrar");
        refs.modalBtnDescargar = document.getElementById("domi-modal-btn-descargar");
        refs.modalFilename = document.getElementById("domi-modal-filename");
        refs.modalRegistros = document.getElementById("domi-modal-registros");
        refs.modalLineas = document.getElementById("domi-modal-lineas");
        refs.modalMonto = document.getElementById("domi-modal-monto");
        refs.modalFecha = document.getElementById("domi-modal-fecha");
        refs.txtPreviewContent = document.getElementById("domi-txt-preview-content");
    }
    ...
    function bindEvents() {
        ...
        refs.btnRemove.addEventListener("click", resetFile);
        refs.btnPreview.addEventListener("click", abrirModalPreview);
        refs.btnGenerar.addEventListener("click", () => generarYDescargar());
        refs.modalClose.addEventListener("click", cerrarModal);
        refs.modalBtnCerrar.addEventListener("click", cerrarModal);
        refs.modalBtnDescargar.addEventListener("click", () => {
            descargarTXT();
            cerrarModal();
        });
    ...
    async function processFile(file) {
        ...
        try {
            ...
            console.log("[DOMI] Total filas:", jsonData.length);
            if (jsonData.length > 0) {
                console.log("[DOMI] Fila 1:", jsonData[0]);
                console.log("[DOMI] Fila 2:", jsonData[1]);
            }
            ...
            for (let i = 0; i < jsonData.length; i++) {
                ...
                let cedula = String(row[0] || "").trim();
                let cuenta = String(row[1] || "").trim();
                let montoRaw = row[2];

                cedula = cedula.replace(/[vVeE\.\-\s]/g, "");
                ...
                if (typeof montoRaw === "number") {
                    ...
                    const montoStr = String(montoRaw)
                        .replace(/[$\s]/g, "")
                        .replace(/\./g, "")
                        .replace(",", ".");
                ...
                if (cedula.length >= 5 && cuenta.length >= 10 && monto > 0) {
                    excelData.push({ cedula, cuenta, monto });
                } else {
                    console.log("[DOMI] Fila descartada", i, "cedula:", cedula, "cuenta:", cuenta, "monto:", monto);
                }
            }

            console.log("[DOMI] Empleados válidos:", excelData.length);

            if (excelData.length === 0) {
                showToast("Error", "No se encontraron registros válidos. El Excel debe tener 3 columnas: Cédula, Cuenta, Monto (sin encabezados)", "error");
                return;
            }
            ...
            mostrarResumen();
            habilitarBotones();
            showToast("Éxito", excelData.length + " empleados cargados (" + (excelData.length * CUOTAS) + " líneas TXT)", "success");

        } catch (err) {
            console.error("[DOMI] Error:", err);
            showToast("Error", "No se pudo leer el archivo Excel: " + err.message, "error");
        }
    }
    ...
    function mostrarResumen() {
        ...
        refs.statLineas.textContent = totalLineas;
        refs.statMonto.textContent = formatNumber(totalMonto);
    }
    ...
    function resetFile() {
        ...
        refs.resumen.style.display = "none";
    ...
    function generarTXT() {
        ...
        if (!fechaNomina) {
            showToast("Error", "Debes seleccionar la fecha de nómina", "error");
            return null;
        }

        const cuentaMatriz = refs.cuentaMatriz.value.trim() || "01770006571102246326";
        const codigoEmpresa = refs.codigoEmpresa.value.trim() || "0139";
    ...
    function abrirModalPreview() {
        ...
        refs.modalFilename.textContent = outputFilename;
        refs.modalRegistros.textContent = excelData.length + " empleados";
        refs.modalLineas.textContent = totalLineas + " líneas";
        refs.modalMonto.textContent = formatNumber(totalMonto) + " Bs";
        refs.modalFecha.textContent = fechaFormateada;

        const lines = txtContent.split("\n").filter(l => l.trim());
        let html = "";

        if (lines.length > 0) {
            html += '<span class="encabezado">' + escapeHtml(lines[0]) + '</span>\n';
            html += '<span class="posicion">  &lt;———— 20 ————→  &lt;— 8 —→  &lt;———— 17 ————→  &lt;4→  &lt;2→</span>\n';
            html += '<span class="posicion">  Cuenta Matriz      Fecha    Monto Total      Reg  Op</span>\n';
            html += '<span class="comentario">  </span>\n';
        }

        const maxPreview = Math.min(lines.length - 1, 7);
        for (let i = 1; i <= maxPreview; i++) {
            html += '<span class="detalle">' + escapeHtml(lines[i]) + '</span>\n';
        }

        if (lines.length > 8) {
            html += '<span class="comentario">  ... (' + (lines.length - 8) + ' líneas más) ...</span>\n';
        }

        html += '<span class="posicion">  &lt;4→  &lt;—— 12 ——→  &lt;———— 20 ————→  &lt;— 10 —→  &lt;5→  &lt;1→  &lt;2→</span>\n';
        html += '<span class="posicion">  Emp  Monto Cuota  Cuenta Empleado    Cédula     Fill  T  Fill</span>';

        refs.txtPreviewContent.innerHTML = html;
    ...
    function descargarTXT() {
        ...
        showToast("Éxito", outputFilename + " descargado correctamente", "success");
    }
    ...
    function showToast(title, message, type) {
        if (typeof window.showToast === "function") {
            ...
            console.log("[" + (type || "info").toUpperCase() + "] " + title + ": " + message);
        }
    }
